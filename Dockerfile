...
RUN mkdir -p /vboxguest && \
    ...
    cp amd64/src/vboxguest-${VBOX_VERSION}/*.ko $ROOTFS/lib/modules/$KERNEL_VERSION.$KERNEL_VERSION_MINOR-tinycore64/ && \
    \
    mkdir -p $ROOTFS/sbin && \
...
RUN depmod -a -b $ROOTFS $KERNEL_VERSION.$KERNEL_VERSION_MINOR-tinycore64
...
RUN curl -L -o $ROOTFS/usr/local/bin/docker https://master.dockerproject.com/linux/amd64/docker && \
